"""Autonomous need-first selection example — F2

MACHINE_NEED: extract plain text from a public PDF URL for indexing
SEARCH_INTENT: extract plain text from PDF
...
REQUIRED_RESULT_FACTS: ['status', 'text', 'document']
PAYMENT FLOW: unpaid probe expects HTTP 402; independently funded buyer settles x402 then retries
AUTOMATIC_NEXT_ACTION: INDEX_TEXT|INGEST_TEXT|SKIP_NO_TEXT
CAMPAIGN_CODE: native_bazaar_f2
...
SEARCH_INTENT = 'extract plain text from PDF'
MAX_PRICE = 0.05
PATH_HINT = '/x402/pdf-text'  # structural filter only — not a brand query
CAMPAIGN = os.environ.get("TASK_RELAY_CAMPAIGN", 'native_bazaar_f2')
...
def main() -> int:
    ...
    print("AUTOMATIC_NEXT_ACTION_AFTER_PAID_RESULT", 'INDEX_TEXT|INGEST_TEXT|SKIP_NO_TEXT')
    return 0
