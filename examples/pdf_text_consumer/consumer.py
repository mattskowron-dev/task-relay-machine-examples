"""Task Relay machine-buyer example — pdf_text_consumer

EXACT MACHINE TRIGGER: Machine needs plain text from a public PDF URL
EXACT TASK RELAY RESOURCE: https://task-relay-production.up.railway.app/x402/pdf-text
EXACT PRICE: 0.01 USDC (atomic 10000)
EXACT REQUEST: see REQUEST_BODY
EXACT STRICT RESULT CONSUMPTION: see consume_result()
EXACT AUTOMATIC MACHINE ACTION: ['INGEST_TEXT', 'SKIP_NO_TEXT', 'ABORT_FETCH_ERROR']
...
RESOURCE = "/x402/pdf-text"
PRICE_USDC = 0.01
ATOMIC = "10000"
CAMPAIGN = os.environ.get("TASK_RELAY_CAMPAIGN", "example_pdf_text_consumer_f2")
REQUEST_BODY = {"url": "https://example.org/document.pdf"}
...
def consume_result(payload: dict) -> str:
    ...
    if task_status != "succeeded":
        action = "ABORT_FETCH_ERROR"
    elif status in ("extracted", "partial"):
        action = "INGEST_TEXT"
    elif status == "no_text":
        action = "SKIP_NO_TEXT"
    else:
...
def main() -> int:
    ...
    try:
        ...
        if e.code == 402:
            ...
            print("automatic_actions_after_paid_result", ['INGEST_TEXT', 'SKIP_NO_TEXT', 'ABORT_FETCH_ERROR'])
            return 0
        print(e.read()[:300])
        return 1
