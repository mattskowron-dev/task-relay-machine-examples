"""Autonomous need-first selection example — F5

MACHINE_NEED: capture a fixed-viewport JPEG of a public page for rendered-page indexing
SEARCH_INTENT: capture website page screenshot JPEG viewport
...
REQUIRED_RESULT_FACTS: ['status', 'artifact', 'render']
PAYMENT FLOW: unpaid probe expects HTTP 402; independently funded buyer settles x402 then retries
AUTOMATIC_NEXT_ACTION: INDEX_RENDERED_PAGE_IMAGE
CAMPAIGN_CODE: native_bazaar_f5
...
SEARCH_INTENT = 'capture website page screenshot JPEG viewport'
MAX_PRICE = 0.25
PATH_HINT = '/x402/page-screenshot'  # structural filter only — not a brand query
CAMPAIGN = os.environ.get("TASK_RELAY_CAMPAIGN", 'native_bazaar_f5')
...
def main() -> int:
    ...
    print("AUTOMATIC_NEXT_ACTION_AFTER_PAID_RESULT", 'INDEX_RENDERED_PAGE_IMAGE')
    return 0
