"""Task Relay machine-buyer example — csv_structure_consumer

EXACT MACHINE TRIGGER: Machine must ingest public CSV into structured rows
EXACT TASK RELAY RESOURCE: https://task-relay-production.up.railway.app/x402/csv-structure
EXACT PRICE: 0.002 USDC (atomic 2000)
EXACT REQUEST: see REQUEST_BODY
EXACT STRICT RESULT CONSUMPTION: see consume_result()
EXACT AUTOMATIC MACHINE ACTION: ['LOAD_ROWS', 'ABORT_INVALID_CSV', 'TRUNCATE_AWARE_CONTINUE']
...
RESOURCE = "/x402/csv-structure"
PRICE_USDC = 0.002
ATOMIC = "2000"
CAMPAIGN = os.environ.get("TASK_RELAY_CAMPAIGN", "example_csv_structure_consumer_f3")
...
def consume_result(payload: dict) -> str:
    ...
    if task_status != "succeeded":
        action = "ABORT_INVALID_CSV"
    elif status == "structured":
        action = "LOAD_ROWS"
    elif status == "partial":
        action = "TRUNCATE_AWARE_CONTINUE"
    else:
        action = "ABORT_INVALID_CSV"
    return action
...
def main() -> int:
    ...
    try:
        ...
        if e.code == 402:
            ...
            print("automatic_actions_after_paid_result", ['LOAD_ROWS', 'ABORT_INVALID_CSV', 'TRUNCATE_AWARE_CONTINUE'])
            return 0
        print(e.read()[:300])
        return 1
