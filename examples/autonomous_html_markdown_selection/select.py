"""Autonomous need-first selection example — F4

MACHINE_NEED: convert HTML already held by the machine into deterministic Markdown
SEARCH_INTENT: convert HTML to Markdown
SELECTION_FILTERS: exact resource path match preferred; price <= MAX_PRICE; required request/result facts
MAX_PRICE: 0.01
REQUIRED_REQUEST_FACTS: ['html']
REQUIRED_RESULT_FACTS: ['status', 'markdown']
PAYMENT FLOW: unpaid probe expects HTTP 402; independently funded buyer settles x402 then retries
AUTOMATIC_NEXT_ACTION: ACCEPT_MARKDOWN|INDEX_MARKDOWN
CAMPAIGN_CODE: native_bazaar_f4
...
SEARCH_INTENT = 'convert HTML to Markdown'
MAX_PRICE = 0.01
PATH_HINT = '/x402/html-to-markdown'  # structural filter only — not a brand query
CAMPAIGN = os.environ.get("TASK_RELAY_CAMPAIGN", 'native_bazaar_f4')
...
def main() -> int:
    ...
    print("AUTOMATIC_NEXT_ACTION_AFTER_PAID_RESULT", 'ACCEPT_MARKDOWN|INDEX_MARKDOWN')
    return 0
