"""Autonomous need-first selection example — F1

MACHINE_NEED: verify one public HTTPS URL is reachable before agent continues work
SEARCH_INTENT: verify public URL health DNS TLS redirects HTTP status
...
REQUIRED_RESULT_FACTS: ['status', 'http_status', 'dns', 'tls']
PAYMENT FLOW: unpaid probe expects HTTP 402; independently funded buyer settles x402 then retries
AUTOMATIC_NEXT_ACTION: ACCEPT_URL|CONTINUE|DEFER|REJECT_TARGET
CAMPAIGN_CODE: native_bazaar_f1
...
SEARCH_INTENT = 'verify public URL health DNS TLS redirects HTTP status'
MAX_PRICE = 0.01
PATH_HINT = '/x402/url-health'  # structural filter only — not a brand query
CAMPAIGN = os.environ.get("TASK_RELAY_CAMPAIGN", 'native_bazaar_f1')
...
def main() -> int:
    ...
    print("AUTOMATIC_NEXT_ACTION_AFTER_PAID_RESULT", 'ACCEPT_URL|CONTINUE|DEFER|REJECT_TARGET')
    return 0
